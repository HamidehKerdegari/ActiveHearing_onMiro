...
class miro_ros_client_gui:
	...
	#   Wait until iiwa is connected zzz!
        if self.miro_soc.isready:
            [Left_e, Right_e, Head, Neck] = self.miro_soc.JointPosition
	    #print [Left_e, Right_e, Head, Neck]
            self.scale_ears_0.set_value(Left_e)
            self.scale_ears_1.set_value(Right_e)

            self.scale_lift.set_value(Neck)
            self.scale_yaw.set_value(Head)
            self.scale_pitch.set_value(50)
...
#                self.spatial_record = self.spatial_record + 1
    ...
    def __init__(self):
	self.miro_soc = server('192.168.1.2', 1234)
        ...
        # bridge config
        self.send_bridge_config = True
        self.no_publish_mics = True
        self.no_publish_cams = True
